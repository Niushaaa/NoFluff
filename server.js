...
You must:
	...
	•	Are interesting, emotional, insightful, or impactful
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 NoFluff Backend running on http://localhost:${PORT}`);
  console.log(`🎬 Video Processing API: http://localhost:${PORT}/api/process/:videoId`);
  
  if (process.env.OPENAI_API_KEY) {
    console.log(`✅ OpenAI API key loaded - Whisper + GPT-4o-mini ready`);
  } else {
    console.log(`⚠️  OpenAI API key not found - check .env file`);
  }
});
